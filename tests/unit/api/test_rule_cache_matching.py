...
def _invalid_regex_issue(issue_id: str = "iss-broken-regex") -> SimpleNamespace:
    return _issue(
        issue_id,
        [
            {
...
def test_invalid_regex_rule_is_isolated_and_reported():
    errors: list[str] = []

    matched = match_issues_for_variant(
        _ctx(),
        [_invalid_regex_issue(), _good_issue()],
        evaluation_errors=errors,
    )

    assert [m.id for m in matched] == ["iss-good"]
    assert errors == ["iss-broken-regex"]

...
현재 설정은 로컬 개발용 기본 비밀번호를 사용하며 PostgreSQL과 pgAdmin을
`127.0.0.1`에만 바인딩합니다. 사내 서버에서도 비밀번호는 반드시 외부에서
주입하고, localhost 바인딩을 유지하거나 승인된 사내 IP만 허용해야 합니다.
...
# 사내망 서버용 — 필요한 IP만 허용
ports:
  - "127.0.0.1:15432:5432"   # localhost only (앱과 같은 서버면)
  # 또는
...
- [ ] PostgreSQL과 pgAdmin 포트가 localhost 또는 승인된 사내 IP에만 바인딩됐는지 확인

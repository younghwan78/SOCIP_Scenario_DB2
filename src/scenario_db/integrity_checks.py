...
def validate_variant_overlay_integrity(
    ...
    ip_by_id = {str(ip.id): ip for ip in ips}
    ...
    for variant in variants:
        ...
        variant_ref = f"{scenario_id}/{variant.id}"

        for node_id, config in (variant.node_configs or {}).items():
            node_id_text = str(node_id)
            path = f"variants.{variant.id}.node_configs.{node_id_text}"
            ...
            if buffer_id_text not in buffer_ids:
                issues.append(
                    IntegrityIssue(
                        ...
                        document_kind="scenario.usecase",
                        document_id=scenario_id,
                        path=f"variants.{variant.id}.buffer_overrides.{buffer_id_text}",

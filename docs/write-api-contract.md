...
- `scenario.import_bundle` stages canonical documents generated by the legacy
  YAML importer or Workbench helper before they are applied to DB tables.
...
`soc.dvfs_table` is a SoC-scoped table version. `dvfs_version` is an independent
sequence per `soc_ref`; `evt_hint` is only metadata that records the guide or
bring-up context. Recalculating the same guide with a newer table creates a new
DVFS version, even when the EVT hint is unchanged.
...
Minimal `soc.dvfs_table` document:
...
- Document kind must be one of `soc`, `soc.dvfs_table`, `ip`, `sw_profile`, `project`, or `scenario.usecase`.
...
- `soc.dvfs_table.soc_ref` must exist in DB or be included as a `soc` document in the same bundle.
- `soc.dvfs_table` must have a non-negative `dvfs_version`.
- `soc.dvfs_table.domains.<key>.domain` must match the domain key.
...
- Documents are applied in dependency order: `soc`, `soc.dvfs_table`, `ip`, `sw_profile`, `project`, then `scenario.usecase`.
- Existing rows are upserted by document ID.
- `soc.dvfs_table` also enforces unique `(soc_ref, dvfs_version)`.
